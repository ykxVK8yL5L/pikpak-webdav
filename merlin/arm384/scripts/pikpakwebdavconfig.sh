...
pikpakwebdav_start_stop(){
    ...
    if [ "${pikpakwebdav_enable}"x = "1"x ];then
        ...
        echo_date "参数为：${pikpakwebdav_port} --pikpak-user ${pikpakwebdav_user} --pikpak-password ${pikpakwebdav_password} --proxy-url ${pikpakwebdav_proxy_url} --root ${pikpakwebdav_root} -S ${pikpakwebdav_read_buffer_size} --cache-size ${pikpakwebdav_cache_size} $AUTH_ARGS" >> $LOG_FILE
        #start-stop-daemon -S -q -b -m -p ${PID_FILE} \
        #  -x /bin/sh -- -c "${BIN} -I --workdir /var/run/pikpakwebdav --host 0.0.0.0 --p ${pikpakwebdav_port} --pikpak-user ${pikpakwebdav_user} --pikpak-password ${pikpakwebdav_password} --proxy-url ${pikpakwebdav_proxy_url} --root ${pikpakwebdav_root} -S ${pikpakwebdav_read_bufffer_size} $AUTH_ARGS >/tmp/pikpakwebdav.log 2>&1"
        ${BIN} -I --no-self-upgrade --workdir /var/run/pikpakwebdav --host 0.0.0.0 --port ${pikpakwebdav_port} --pikpak-user ${pikpakwebdav_user} --pikpak-password ${pikpakwebdav_password} --proxy-url ${pikpakwebdav_proxy_url} --root ${pikpakwebdav_root} -S ${pikpakwebdav_read_buffer_size} --cache-size ${pikpakwebdav_cache_size} $AUTH_ARGS >/tmp/upload/pikpakwebdav.log 2>&1 &
        sleep 5s
